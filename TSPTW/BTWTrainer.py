...
from DeCoModel import TSPModel as Model
from test import main_test
from BTWEnv import BTWEnv as Env
...
class TSPTrainer:
    ...
    def run(self):
        ...
        self.env.load_all_dataset(self.env_params['data_path'],self.env_params['solution_path'] )#VAS
    ...
    def _train_one_batch(self, episode,batch_size):
        ...
        while not done:
            if current_step == 0:
                selected_teacher = self.env.solution_batched[:, 0] # starting node
                selected_student = self.env.solution_batched[:, 0]
                prob = torch.ones(self.env.solution_batched.shape[0], 1)      
            else:
                selected_teacher, prob, _, selected_student = self.model(state, self.env.selected_node_list, self.env.solution_batched, current_step,tw_norm,tw_mask,False)  # 更新被选择的点和概率
        ...
        loss_return = -prob_list.log().mean()
        return 0,0, loss_return.item()
