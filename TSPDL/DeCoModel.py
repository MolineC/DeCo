...
class TSPModel(nn.Module):
    ...
    def forward(self, state, selected_node_list, solution, current_step,tw_norm = True,tw_mask = None,greedy = True):
        ...
        node_tw=state.node_dl
        dummy_relative_zero = torch.zeros_like(state.current_load)
        if tw_norm:
        ...
        if self.mode == 'train':            
            ...
            encoded_tw = self.tw_encoder(node_tw,state.current_load)
            self.encoded_nodes = self.fusion(torch.cat((encoded_nodes,encoded_tw),dim=2))
            probs = self.decoder(self.encoded_nodes, selected_node_list,tw_mask,dummy_relative_zero)
            selected_student = probs.argmax(dim=1)  # shape: B
            selected_teacher = solution[:, current_step]  # shape: B
            prob = probs[torch.arange(batch_size_V, device=probs.device)[:, None], selected_teacher[:, None]].reshape(batch_size_V, 1)  # shape: [B, 1]
        if self.mode == 'test':
                if current_step <= 1:
...
class TW_Encoder(nn.Module):
    ...
    def forward(self, data,current_time):
        ...
        processed = data - current_time_expanded #
        result =processed
...
class TSP_Decoder(nn.Module):
    ...
    def _get_new_data(self, data, selected_node_list, problem_size, B_V,mask):
        ...
        unselected_len = problem_size - list.shape[1]  # 
      
        index_2 = list.to(device=data.device, dtype=torch.long)
    ...
    def forward(self,data,selected_node_list,mask = None,current_time=None):
        ...
        # left avaliable node's shape: [B, V-current_step]
        ...
        props_clone[index_small] = props_clone[index_small] + torch.tensor(1e-7, dtype=props_clone[index_small].dtype, device=props.device)  # prevent the probability from being too small
        props = props_clone

        All_props = torch.zeros(batch_size_V, problem_size, device=data.device, dtype=props.dtype)
        ...
        index_2_ = selected_node_list.to(device=data.device, dtype=torch.long)  
        All_props[index_1_, index_2_] = -2
        
        index=All_props!=-2
        All_props[index] = props.ravel()
        All_props[index_1_,index_2_]=0
        # All_props=F.softmax(All_props,dim=-1)
        # All_props[index_1_, index_2_]=All_props[index_1_, index_2_] -1
        return All_props
...
def multi_head_attention(q, k, v, attn_mask=None, dropout_p=0.0, is_causal=False):
    """
    FlashAttention/SDPA version of multi-head attention.

    Args:
        q, k, v: tensors with shape [batch, head_num, seq_len, head_dim].
        attn_mask: optional attention mask accepted by
            torch.nn.functional.scaled_dot_product_attention.
        dropout_p: attention dropout probability. Keep 0.0 for inference/eval.
        is_causal: whether to apply causal masking. This model uses full attention,
            so the default is False.

    Returns:
        Tensor with shape [batch, seq_len, head_num * head_dim], same as the
        original hand-written attention implementation.
    """
    batch_s = q.size(0)
    head_num = q.size(1)
    n = q.size(2)
    ...
    # PyTorch SDPA will dispatch to FlashAttention / memory-efficient attention
    # on supported CUDA hardware and dtypes, otherwise it safely falls back.
    out = F.scaled_dot_product_attention(
        q, k, v,
        attn_mask=attn_mask,
        dropout_p=dropout_p,
        is_causal=is_causal,
    )  # shape: [B, head_num, n, head_dim]

    out_concat = out.transpose(1, 2).contiguous().view(batch_s, n, head_num * key_dim)
